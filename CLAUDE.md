...
**2026-06-19: GPT版の実データ（テスター本人がP001としてSet A〜D実施）を独立分析した結果、GPT版の自己採点には疑わしい点が見つかった**（Set B/C/Dで「時制」カテゴリの`obligatory_contexts`/`error_count`/`error_rate`が3回とも完全に同じ数値＝13/3/23.08%で一致しており、毎回まじめに数え直しているか疑わしい。また「名詞の境界（冠詞）」エラーを毎回過小評価している可能性が高い）。これを受けて以下の精度改善策を実装した。

- **テスターログイン機能を追加**：テスターごとの個別ユーザー名・パスワードで認証（後述6.4節）
...
**化石化条件（2026-06-22に確定・GPT版/Gemini版両方に適用）：観点別エラー率 ≥ 全体平均 + 15ポイント のみ。**
GPT-4o時代から引き継いでいた「全体平均 ≤ 40%」という絶対値ゲートは撤廃した。実データで試したところ、ゲート＋30ポイントの組み合わせでは化石化がほとんど検出されず（特に全体平均が40%を超える学習者は構造的に絶対検出されない）、テスター実験向けにはもっと拾えるようにすべきと判断し緩和した。+30ポイント→+15ポイントへの変更も同時に行っている。
この閾値は人間の手動添削データとの比較検証（§8参照）を経た値ではなく、テスター実験向けの**ひとまずの仮決め**である。
...
スプレッドシートに蓄積された、あるテスターIDの**全テスト・全セットを横断して集計**し、以下のルールでCEFRレベルを推定する。
...
pages/dashboard.py                  # テスターごとの統合分析ダッシュボード（CEFR判定・推移グラフ）
...
**Testers タブ**：テスターログイン用の認証情報。`username` / `password` の2列（ヘッダー行あり）。`app.py`の`get_tester_credentials()`が`st.cache_data(ttl=300)`で5分キャッシュして読む。テスターを追加する際は、このタブに直接行を追記すればよい（コード変更不要）。
...
`tester_password` / `admin_password` のような共通パスワードは使わない（テスターごとの個別ユーザー名・パスワードはsecretsではなくSheetsの`Testers`タブで管理。6.4節）。`OPENAI_API_KEY`はもう使わないので削除してよい。
...
`English_AI_Logs_Gemini`内の`Testers`タブ（A列`username`/B列`password`、ヘッダー行あり）に行を追加するだけでテスターを登録できる。コード変更・再デプロイは不要。ログインすると、そのユーザー名がそのままテスターID（`st.session_state.user_id`）として使われる（自由入力のテスターID欄は廃止）。パスワードは平文でシートに保存される簡易的な仕組みであり、外部の無関係なアクセスを防ぐためのものであって、強固な認証ではない。
...
- **Gemini無料枠のレート制限（RPM）**：1分あたりのリクエスト数に上限があるため、複数テスターが同時にテストを行うと429エラーが起こりうる。1問あたりのGemini呼び出しが2段階パイプライン化により2回（文字起こし＋ノイズ補正）に増えたため、従来よりRPMを消費しやすい。`call_gemini()`でリトライ（指数バックオフ）は実装済みだが、上限を大きく超える同時アクセスには無対応。
...
- **テスターログインのパスワードは平文**：`Testers`タブにそのまま平文で保存している。第三者の不正利用を防ぐための簡易的な仕組みであり、強固な認証ではない。
- **CEFR判定はテスターの全テスト履歴を累積**して算出されるため、1回のテストだけでは結果が安定しない可能性がある。
...
3. 化石化閾値（+15ポイント）の感度分析・妥当性検証（人間の手動添削データとの比較を経て確定する予定）
4. 複数テスターの同時アクセスに対するレート制限対策の強化（キューイング等）
